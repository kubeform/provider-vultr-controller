...
// MetalServerReconciler reconciles a MetalServer object
type MetalServerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bare.vultr.kubeform.com,resources=metalservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bare.vultr.kubeform.com,resources=metalservers/status,verbs=get;update;patch

func (r *MetalServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("metalserver", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MetalServer")
...
func (r *MetalServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &barev1alpha1.MetalServer{}); err != nil {
			klog.Error(err, "unable to set up auditor", barev1alpha1.MetalServer{}.APIVersion, barev1alpha1.MetalServer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&barev1alpha1.MetalServer{}).
...
func (r *MetalServerReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for MetalServer")
			return nil
		}
		...
		resourceList := &barev1alpha1.MetalServerList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list MetalServer")
			return nil
		}
