...
// +kubebuilder:rbac:groups=kubernetes.vultr.kubeform.com,resources=kubernetes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kubernetes.vultr.kubeform.com,resources=kubernetes/status,verbs=get;update;patch

func (r *KubernetesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("kubernetes", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Kubernetes")
...
func (r *KubernetesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kubernetesv1alpha1.Kubernetes{}); err != nil {
			klog.Error(err, "unable to set up auditor", kubernetesv1alpha1.Kubernetes{}.APIVersion, kubernetesv1alpha1.Kubernetes{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kubernetesv1alpha1.Kubernetes{}).
