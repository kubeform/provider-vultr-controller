...
// NodePoolsReconciler reconciles a NodePools object
type NodePoolsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kubernetes.vultr.kubeform.com,resources=nodepools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kubernetes.vultr.kubeform.com,resources=nodepools/status,verbs=get;update;patch

func (r *NodePoolsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("nodepools", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NodePools")
...
func (r *NodePoolsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kubernetesv1alpha1.NodePools{}); err != nil {
			klog.Error(err, "unable to set up auditor", kubernetesv1alpha1.NodePools{}.APIVersion, kubernetesv1alpha1.NodePools{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kubernetesv1alpha1.NodePools{}).
