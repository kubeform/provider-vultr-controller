...
// PrivateReconciler reconciles a Private object
...
// +kubebuilder:rbac:groups=iso.vultr.kubeform.com,resources=privates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iso.vultr.kubeform.com,resources=privates/status,verbs=get;update;patch

func (r *PrivateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("private", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Private")
...
func (r *PrivateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &isov1alpha1.Private{}); err != nil {
			klog.Error(err, "unable to set up auditor", isov1alpha1.Private{}.APIVersion, isov1alpha1.Private{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&isov1alpha1.Private{}).
