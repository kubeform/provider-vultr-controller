...
// +kubebuilder:rbac:groups=reverse.vultr.kubeform.com,resources=ipv6s,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=reverse.vultr.kubeform.com,resources=ipv6s/status,verbs=get;update;patch

func (r *Ipv6Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipv6", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ipv6")
...
func (r *Ipv6Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &reversev1alpha1.Ipv6{}); err != nil {
			klog.Error(err, "unable to set up auditor", reversev1alpha1.Ipv6{}.APIVersion, reversev1alpha1.Ipv6{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&reversev1alpha1.Ipv6{}).
