...
// +kubebuilder:rbac:groups=startup.vultr.kubeform.com,resources=scripts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=startup.vultr.kubeform.com,resources=scripts/status,verbs=get;update;patch

func (r *ScriptReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("script", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Script")
...
func (r *ScriptReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &startupv1alpha1.Script{}); err != nil {
			klog.Error(err, "unable to set up auditor", startupv1alpha1.Script{}.APIVersion, startupv1alpha1.Script{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&startupv1alpha1.Script{}).
